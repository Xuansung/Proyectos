...
//MODIFICACIÓN
// Clase ABE (Árbol binario equilibrado)
template <class Key>
class ABE : public AB<Key> {
 public:
  ABE(bool traza = false) : AB<Key>(traza) {}
  virtual ~ABE() {}
  virtual bool insertar(const Key& key) override {
    bool grew = false;
    bool inserted = false;
    this->raiz = insertarABE(this->raiz, key, grew, inserted);
    return inserted;
  }
  // Recorrido inorden del ABE
 ...
 private:
  NodoB<Key>* insertarABE(NodoB<Key>* nodo,const Key& key, bool& grew, bool& inserted) {
    if(!nodo) {
    ...
    NodoAVL<Key>* nodoAVL = static_cast<NodoAVL<Key>*>(nodo);
    if(nodoAVL->bal < 0) {
      nodoAVL->izdo = insertarABE(nodo->izdo, key, grew, inserted);
      if(grew) {
        nodoAVL->bal += 1;
      }
    } else {
      nodoAVL->dcho = insertarABE(nodo->dcho, key, grew, inserted);
      if(grew) {
        nodoAVL->bal -= 1;
      }
    }
