...
const char* opcodes[] = { "00010001", "00000010", "00000011", "00000001", "00100010", "00100011", "00100000", "00100001", "00100100", "00100101", "00100110", "00100111", "00000000", "00010010", "00010011", "11111111", "00000100", "00000101", "00000110", "00000111", "00001000", "00001001", "00110000", "00110011" };
...
const int posoper[NUMINS][MAXNUMOPER] = { {23, 19, 0},
                                         {15, 0, 0},
                                         {15, 0, 0},
                                         {15, 0, 0},
                                         {23, 7, 3},
...
int processLine(FILE* file, bool *code, int *currentline, int counter) {
        ...
        if (c == ';') { //Comentario, no hay nada más hasta final de línea, puede acabar nemonico sin operandos
            if (isMnemonic) { //Si estabamos procesando la cadena de un nemonico
                *ptchar = '\0'; //Acaba la cadena en line
                ...
                ungetc(c, file);
                if (!eatComment(file)) { //EOF, salimos indicando que posiblemente hay línea
    ...
    exit(1);
    //Esta vuelta no se alcanzarà, solo con un nemonico o etiqueta excesivamente grande
}
...
void ensambla(const char* srcfilename, const char* dstfilename)
{
    ...
    if (fopen_s(&infile, srcfilename, "rb") != 0)
    {
...
int main(int argc, char* argv[]){
    ...
    // Si se especifican dos argumentos, el primero es el archivo de entrada y el segundo el de salida
    else if (argc == 3) {
        inputFile = argv[1];
        outputFile = argv[2];
    }
    // Si hay más de dos argumentos, informar al usuario sobre el uso correcto del programa
    else if (argc > 3) {
